...
\begin{definition}
  ...
  $$
    \aruns(\prog) \defeq 
    \{ \store \mid \preproc(\store)  \wedge ((\store_{S\cup R},\prog) \aredx^* (\store,\varnothing)\}% \vee (\store_{S \cup R},\prog) \aredx^* (\store,\bot)) \}
  $$
  and also define the following which pads out undefined views and outputs with $\bot$:
  $$
  \begin{array}{l}
    \botruns(\prog) \defeq \\
