...
$\minifed$ with abort $\bot$ and an \emph{assert} command.
\begin{definition}
  We add $\bot$ to our syntax of programs and add the form
  $\elab{\assert(\phi(\be))}{\cid}$ to the command syntax of $\minifed$,
  with semantics given in Figure \ref{fig-adversary}. We also extend the
  domain of memories $\store$ to $\mathbb{F}_p \cup \{ \bot \}$.
\end{definition}
