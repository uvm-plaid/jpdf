...
\begin{definition}
  ...
  of variables $X_1 \subseteq X_2$ in $\pmf$, denoted $\margd{\pmf}{X_1}$, is defined as follows:
  $$
  \forall \store \in \mems(X_1) \quad . \quad \margd{\pmf}{X_1}(\store) =
  \sum_{\store' \in \mems(X_2-X_1)} \pmf(\store \uplus \store')
  $$
\end{definition}

\begin{definition}
  Given $\pmf$ with $\dom(\pmf) = \mems(X_3)$, the \emph{conditional distribution}
  of $X_1$ given $X_2$ where $X_1 \cup X_2 \subseteq X_3$ and $X_1 \cap X_2 = \varnothing$, denoted $\condd{\pmf}{X_1}{X_2}$,
  is defined as follows:
  $$
  \forall \store \in \mems(X_1 \cup X_2)\ .\ 
  \condd{\pmf}{X_1}{X_2}(\store) =
  \begin{cases}
    0 \text{\ if\ } \margd{\pmf}{X_2}(\store_X_2) = 0\\
    \margd{\pmf}{X_1 \cup X_2}(\store) / \margd{\pmf}{X_2}(\store_X_2)\ \text{\ otherwise}
  \end{cases}
  $$
\end{definition}
...
\begin{definition}
  Given $\pmf$, for all $\store_2 \in \mems(X_2)$ and $\store_1 \in \mems(X_1)$ define:
  \begin{enumerate}
