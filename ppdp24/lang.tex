...
(partial) mapping from variables $x$ to values $v \in \mathbb{F}_p$.
The \emph{domain} of a memory is written $\dom(\store)$ and is
the finite set of variables on which the memory is defined.
We write $\store\{ x \mapsto v\}$ for $x\not\in\dom(\store)$
to denote the memory $\store'$ such that
...
\store'(x)$ for all $x \in \dom(\store)$. We write $\store \uplus
\store'$ to denote the combination of $\store$ and $\store'$
assuming $\store(x) = \store'(x)$ for all $x \in \dom(\store)
\cap \dom(\store')$, otherwise $\store \uplus \store'$ is undefined.
\cnote{Check to make sure all $\cap$ and $\cup$s have been revised
with $\uplus$.}
We write $\store \subseteq \store'$ iff $\store \uplus \store'
= \store$.
