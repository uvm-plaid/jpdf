\section{Verification of Protocols in $\mathbb{F}_2$}
...
the size of $\secrets(\prog) \cup \randos(\prog)$, this strategy is
...
As per standard nomenclature, \emph{atoms} are $\minifed$ variables $x$, \emph{literals}
are atoms or negated atoms, and clause bodies are conjunctions of literals.
A \emph{fact} is a clause with no body. A \emph{Datalog program} is a list of clauses.
$$
\begin{array}{rclr}
...
program is to apply $\solve$ locally to each command
...
a fact is a clause of the form $x \gets \varnothing$ and means that $x$
...
\begin{definition}
  ...
  Then define:
  $$
  \mathit{facts}(\store) = x_1 \gets \varnothing, \ldots, x_n \gets \varnothing
  $$
\end{definition}
