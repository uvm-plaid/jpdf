...
Our work provides a complete methodology for end-to-end verification
...
\item A \textbf{fully-automated verification method} for low-level MPC
  primitives in $\mathbb{F}_2$ (\S\ref{section-bruteforce}).
\item A \textbf{partially-automated verification method} for
  high-level MPC protocols, which leverages automated proofs for
  low-level primitives (\S\ref{section-example-gmw}).
\end{enumerate}
%
