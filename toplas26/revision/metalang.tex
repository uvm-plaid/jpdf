...
will discuss in Sections \ref{section-metaty} and especially
\ref{section-examples} where we present and discuss
extended examples of $\metaprot$ programs.
...
$x$. $\metaprot$ \emph{expression} forms allow dynamic construction of these
values. Note in particular that $\concat$ is string concatenation.
...
While type inference for this simple type system would be possible,
it would complicate the novel theory and implementation elements
of our presentation, so we just check annotations instead.

Formally, we consider a complete metaprogram to include
both a codebase and a ``main'' program that uses the codebase.
For all functions (both the expression and instruction forms) we disallow
recursion, which allows us to guarantee the decidability
of type dependence (Section \ref{section-metaty}).
\begin{definition}
...
$\metaprot$ instruction functions can optionally be annotated with pre
and postconditions. To model this in our theory we posit functions
$\precond$ and $\postcond$ that specify conditions for instruction
functions $f \in \dom(\codebase)$.  Note that $\precond(f)$ or
$\precond(f)$ may be undefined if no annotation is given for $f$. In
actual code, we write pre and postconditions directly before and after
functions to define them. Here is a simple example function
$\ttt{bar}$ with pre and postcondition annotations. Note that the
annotations are abstract with respect to the input string $\ttt{x}$,
and that string concatenation can be used in pre and postcondition
constraints:
\begin{verbatimtab}
...
\emph{abstract constraints} as $\peq$ with the syntax given
...
GMW $\ttt{and}$ gates \cite{evans2018pragmatic} implementations is
given in Figure \ref{fig-gmwprepost}. Here, the $\ttt{xorgmw}$ and
$\ttt{andgmw}$ functions takes as input strings $x$ and $y$ that
distinguish values secret-shared between clients 1 and 2 as
$\mx{x}{1}, \mx{x}{2}$ and $\mx{y}{1},\mx{y}{2}$ respectively-- that
is, these shares encode $\ttt{RECON}(x)$ and $\ttt{RECON}(y)$.  The
postcondition of the non-interactive $\ttt{xorgmw}$ protocol specifies
that it results in a value shared between clients 1 and 2 as
$\ttt{RECON}(z)$ that is the xor-ing of $\ttt{RECON}(x)$ and
$\ttt{RECON}(y)$. The postcondition of the interactive protocol
defined by the $\ttt{andgmw}$ specifies that it results in a value
shared between clients 1 and 2 as $\ttt{RECON}(z)$ that is the
conjunction of $\ttt{RECON}(x)$ and $\ttt{RECON}(y)$. The
postcondition for $\ttt{andgmw}$ especially is non-trivial to prove,
and expressing these postconditions in a compositional manner reduces
proof overhead and improves scalability to large circuits that may
incorporate a multitude of $\ttt{and}$ gates.

\begin{fpfig}[t]{GMW $\ttt{and}$ gates with postcondition.}{fig-gmwprepost}
\begin{verbatimtab}
RECON(x) { (m[x]@1 + m[x]@2) }
...
andtablegmw(x:string, y:string , z:string) {
...
\begin{lemma}
  \label{lemma-fresh}
  Given $\eqs$ and $w, \cid = \fresh(\stringty, \cidty)$ and
  some $w',\cid'$ where 
...
\begin{proof}
  ...
  variables in $\eqs'$ such that $\mathit{rename}(\eqs) = \eqs'$. Suppose
  on the contrary that 
  $\store' \models \eqs'$ and $\store' \not\models \eqs$, and define $\store$ such that:
  $$
...
and postconditions to be defined, since the analysis will infer valid
pre and postconditions in their absence by rule \TirName{Fn}, but
their intent is to enable fine-tuning of pre and postconditions for
improving verification efficiency as discussed above and later in
Section \ref{section-examples}.  An important implementation detail is
that verification of any given function $f$ only needs to be performed
once, and can then be memoized for other applications using the
\TirName{App} rule as a consequence of Lemma \ref{lemma-genentails}
...
for all $[\mv_1/y_1]\cdots[\mv_n/y_n]$ with $\mv_1 : \tau_1, \ldots
\mv_n : \tau_n$ there exists $\eqs_1$, $\eqs_2$
where:
\begin{mathpar}
...
\subsection{Abstact Hoare Logic Soundness}
...
\begin{proof}
  ...
  \end{mathpar}
  and by inversion of the $\TirName{Seq}$ rule we have $\htrip{\peq_1^1}{\cmd_1}{\peq_2^1}$ and $\htrip{\peq_1^2}{\cmd_2}{\peq_2^2}$,
  %\begin{mathpar}
  %  \htrip{\peq_1^1}{\cmd_1}{\peq_2^1}
  %
  %  \htrip{\peq_1^2}{\cmd_2}{\peq_2^2}
  %\end{mathpar}
  and by the induction hypothesis we have $\htrip{\eqs_1^1}{\cmd_1}{\eqs_2^1}$ and $\htrip{\eqs_1^2}{\cmd_2}{\eqs_2^2}$.
  %\begin{mathpar}
...
\begin{corollary}
  ...
  $\htrip{\eqs_1}{\prog}{\eqs_2}$ is valid.
\end{corollary}
