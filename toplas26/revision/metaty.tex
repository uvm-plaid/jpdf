...
\label{sec:metapr-type-check}

\stafig

Now we develop a compositional type system for $\metaprot$ based on 
$\minicat$ typing and the automated Hoare logic developed in
Section \ref{section-metalang}. We focus on share types for confidentiality
which is more complex and general than our integrity type system.
An algorithmic approach to the latter was developed previously for
$\metaprot$ \cite{skalka-near-esop25}, and also the core element
of the system-- BDOZ/SPDZ-style MAC checking-- can be treated as
algebraic correctness conditions enforceable by Hoare triples
as we show in Section \ref{section-examples}.

\subsection{Share Type Checking with Annotations}
\label{section-metaty}

Share types rely on SMT entailments via the \TirName{ShareEntails} and
\TirName{PubEntails} rules, which are not syntax directed, and likewise
the interactive typing judgement is nondeterministic. Our goal
...
defines an SMT equivalence-- the key idea here being that explicit
equivalences can stand in for implicit \TirName{ShareEntails} and
\TirName{PubEntails} judgements-- and $\notg{\stt} : \sty$ which
defines a share type assertion verified using the automated $\Vdash$
typing judgement. In the $\metaprot$ semantics type annotations are
interpreted as $\pskip$s, while their integration in Hoare triple
...
respectively). In these rules we use $\xdefs$ to recover defined
...
\noindent We also introduce dependent $\Pi$ types for functions, that allow
...
$\notg{\Gamma_1}$, application of the function results in the bindings
in $\notg{\Gamma_2}$ using the random values in $\notg{R}$ for share
generation or secure openings.  We require that the type is closed,
i.e., only $y_1,\ldots,y_n$ can occur free in $\notg{\Gamma_1}$,
$\notg{\Gamma_2}$ $\notg{R}$.  Type checking for functions is
accomplished by using fresh ``witness'' values of $\stringty$ and
$\cidty$ types to instantiate dependent variables, similarly to the
\TirName{GenEntails} rule. As defined in the \TirName{App} rule,
typing ensures that application of the function instantiates the
dependencies, checks that the instantiation of $\notg{\Gamma_1}$ is
contained in the share typing environment, and adds the instantiation
of $\notg{\Gamma_2}$ as an effect along with instantiation of
$\notg{R}$ with linearity enforced. A practical programming detail is
where the share type environment precondition $\notg{\Gamma_1}$ is
defined by the programmer-- by convention we write these as
additional components of \ttt{precondition} annotations, as
illustrated in the extended examples discussed in Section
...
we note that the $\ttt{xorgmw}$ gate needs no type annotations for type checking.
...
is needed to specify that the gate output wire values are secret-shared using
...
the (sub)protocol it defines interactively generates new shares using an OTP
($r[z]@1$).
...
Demonstrating soundness of $\metaprot$ type checking is similar to $\minicat$--
we show the ${OK}$ invariant is preserved by the algorithm. By Theorem \ref{theorem-OK}
soundness follows. Lemma \ref{lemma-metaty} follows by appeal to Lemma \ref{lemma-sty-noninteractive-sound}
since the algorithmic judgement $\Vdash$ is sound with regard to the noninteractive
typing rules by definition. Theorem \ref{theorem-metaty} follows as a special
case of Lemma \ref{lemma-metaty}.
\begin{lemma}
  \label{lemma-metaty}
  Given $\prog_1,\Gamma_1,R_1 : {OK}$ and $\cmd \redx \prog_2$ and $\htrip{\eqs_1}{\cmd}{\eqs_2}$ with
  $\eqs_1$ satisfiable and $\toeq{\prog_1} \models \eqs_1 \wedge \toeq{\xdefs_1}$ and
  $\shtrip{\Gamma_1,R_1,\xdefs_1}{\cmd}{\Gamma_2,R_2,\xdefs_2}$.
  Then $\prog_1;\prog_2,\Gamma_2,R_2 : {OK}$ and $\toeq{\prog_1;\prog_2} \models \toeq{\xdefs_2}$.
\end{lemma}

\begin{theorem}
  \label{theorem-metaty}
  Given $\htrip{\eqspre}{\ttt{main()}}{\eqs}$ and $\ttt{main} : \Gamma_1 \rightarrow (\Gamma_2,R)$
  and $\main() \redx \prog$ and $\progpre,\Gamma_1,R' : {OK}$ %with $R' \cap R = \varnothing$
  and $\toeq{\progpre} \models \eqspre$. Then $(\progpre;\prog),\Gamma_2,R \uplus R': {OK}$.
\end{theorem}

Note that Theorem \ref{theorem-metaty} assumes the existence of
$\progpre$ with $\progpre,\Gamma_1,R' : {OK}$ and $R' \cap R =
\varnothing$ and $\toeq{\progpre} \models \eqspre$. The assumption of
some preprocessing protocol such as Beaver triple or MAC share
distributions is typical, and our approach does not require the
protocol to be defined by the programmer, but rather just (weakened)
postconditions of it that are sufficient to verify the protocol being
programmed.

Theorem \ref{theorem-metaty} also does not provide a complete recipe
for obtaining noninterference modulo output-- as per Theorem
\ref{theorem-sty}, it is also necessary to check closure of
$\Gamma_2$. There are various ways this can be accomplished for the
$\metaprot$ analysis-- for example, we evaluate $\ttt{main}$ to obtain
all program variables and then check closure of $\Gamma_2$ as
prescribed by Definition \ref{def-sty-closed}. We can also add checks
to the type system to guarantee that all messages exchanged between
clients are classified when type checking functions, which is
a simple addition.
