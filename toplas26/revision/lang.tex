...
a finite field. Synchronous means that the sender and receiver in each
communication ``block'' until data is exchanged. This is consistent
with standard models of MPC and real protocols \cite{evans2018pragmatic}.
In order to align with existing SMT theories
\cite{SMFF} we restrict our consideration to prime fields. We identify
clients by natural numbers and federations- finite sets of clients-
are always given statically.  Our threat model assumes a partition of
the federation into \emph{honest} $H$ and \emph{corrupt} $C$
subsets. We model probabilistic programming via a random tape
semantics. That is, we will assume that programs can make reference to
values chosen from a uniform random distribution defined in the
...
\item $\eassign{\mesg{w}}{\cid_2}{\be}{\cid_1}$: This
  is a unicast \emph{message send} where expression $\be$ is computed
...
formulations, e.g., \cite{6266151}. While this distinction is important,
it will frequently be helpful to refer to located variables in general
so we collect them in a syntactic sort $x$. To help identify and distinguish
between collections of variables in protocols we introduce the
following notation.
\begin{definition}
We let $x$ range over \emph{located variables} which are identifiers where
...
from \emph{configurations} $(\store,\prog)$ to final memories
...
implementation. In particular, we do not define a semantics of OT in
Figure \ref{fig-minicat-redx}. However we will be able to define a
formal specification of OT-- we return to this in Section
\ref{section-smt}. Informally, any use is of the form $\mx{w}{\cid_1}
:= \elab{\OT{\be_1}{\cid_1}{\be_2}{\be_3}}{\cid_2}$ -- given a
\emph{choice bit} $\be_1$ provided by a receiver $\cid_1$, the sender
$\cid_2$ sends either $\be_2$ or $\be_3$. Critically, $\cid_2$ learns
nothing about $\be_1$ and $\cid_1$ learns nothing about the unselected
value.
...
the protocol is aborted. Note that abort is an explicit action, not
semantic ill-formedness (disallowed by sanity conditions). To model
this, we extend $\minifed$ with an \ttt{assert} command:
$$
...
a memory $\store$ or $\bot$. Note that any corrupted party is free to
ignore any failing $\ttt{assert}$, in addition to rewriting their
own protocol steps.
...
\cite{evans2018pragmatic,10.1007/978-3-030-68869-1_3}. We therefore
posit a predicate $\preproc$ on memories that specifies relevant
preconditions, e.g., that it contains Beaver triples with
associated properties, or pre-distributed keys for MAC checking.

We define $\runs(\prog)$ as the set of final memories resulting from
execution of $\prog$ given any initial memory, possibly augmented with
a preprocessed memory (a memory that satisfies a predicate
$\preproc$), and $\aruns(\prog)$ is the analog in the malicious
setting. It is necessary to define $\aruns$ independently since
adversaries can rewrite their own protocol steps and thus affect
protocol semantics. Since we constrain programs to not overwrite any
variable, we are assured that final memories contain both a complete
record of all initial secrets as well as views resulting from
communicated information. This formulation will be fundamental to our
consideration of probability distributions and hyperproperties of
protocols.
%In this
...
\begin{definition}
  ...
  \end{array}
  $$
  And similarly for any adversary $\adversary$ we define adversarial runs
  as follows:
  $$
  \begin{array}{c}
    \aruns(\prog) \defeq 
