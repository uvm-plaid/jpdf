...
about the underlying definitions of $\ttt{genshares}$, $\ttt{xorgmw}$,
...
it as a hyperproperty that implies passive simulator
security.
...
Compared to this prior whole-program analysis, our use of weaker
customized postconditions supports compositional reuse of verified
components. Section \ref{section-examples} quantifies the resulting
...
\node[analysis, dashed] (prelude-integrity) at (0,-1.0)
  {Automated integrity\\type checking\\Prior work~\cite{skalka-near-esop25}};
...
\node[group, fit=(prelude)(prelude-hoare)(prelude-types)(prelude-integrity),
...
\draw[flow] (prelude-integrity) -- node[edge label] {preserves} (integrity-types);
...
ANTLR4 to parse Prelude the source code, Scala to convert the Prelude abstract
...
includes whether the language system provides \emph{probabilistic
...
whole-program analysis. The type systems presented in these works
...
\item We implement and evaluate the compositional verification of
  ...
  empirical advantages of compositional vs. whole-program verification.
\end{itemize}

The SAW/Cryptol framework \cite{10.1007/978-3-319-48869-1_5} supports
definitions of cryptographic protocols and verification of functional
correctness using SMT. The Swanky extension \cite{swanky} is
especially related to our work, since it is intended to apply to MPC
protocols in binary and prime fields. For example, Swanky has been
used in the implementation of ZK systems
\cite{cryptoeprint:2020/1410}. However, while the goals of our system
are overlapping with regard to verification of protocol correctness,
we are also focused on type systems for enforcement of security
properties, incorporating reasoning about probabilistic behavior of
protocols. SAW/Cryptol and Swanky, by contrast, are not designed to
verify security properties of protocols, since these are inherently
probabilistic. Our language model also more readily accommodates
primitives for message passing between multiple parties. But as we
discuss in Section \ref{section-conclusion}, leveraging Swanky to
accelerate verification is an interesting direction for future work.
