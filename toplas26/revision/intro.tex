...
functionality} and a standard approach is to implement ideal
functionalities as either arithmetic or boolean circuits. An advantage
of this is that subprotocols for primitive operations, such as
addition and multiplication, or logic gates, can be implemented and
optimized as subprotocols. Consider the example of a two-bit adder
binary circuit in Figure \ref{fig-adder}.
...
previous work \cite{skalka-near-ppdp24, skalka-near-esop25}. The prior work developed the 
...
Together, our contributions \emph{automate} the \emph{compositional} verification of security
...
Hoare logics and type systems for compositional verification, and use SMT to automate the checks
...
% \end{enumerate}
\begin{enumerate}[i.]
\item In Section~\ref{section-smt}, a Hoare logic for Overture protocols, for the compositional verification of both semi-honest and malicious security.
\item In Section~\ref{section-sty}, a novel share type system, enabling security verification for protocols that use secret sharing as a building block; our approach works for any secret sharing scheme and enforces passive security (confidentiality).
\item In Section~\ref{section-ipj}, an integrity type system for extending passive security to malicious security.
\item In Section~\ref{section-metalang}, a Hoare logic for Prelude programs with an automated implementation, for the compositional verification of high-level protocol specifications.
\item In Section~\ref{sec:metapr-type-check}, a compositional type-based verification approach for Prelude programs that combines share types with the automated Hoare logic of Section 7.
\end{enumerate}



A prior conference version of this work~\cite{skalka-near-esop25}
proposed a whole-program analysis for Prelude programs that ensures
confidentiality, and combined it with an integrity type system to
prove security of Prelude programs. This extended version adds the
compositional Hoare logics presented in Sections 4 and 7, adds the
share type system presented in Section 5 that enforces a stronger
security property than the type system in~\cite{skalka-near-esop25},
and fully develops the compositional type-based verification approach
in Section 8. In addition, we develop an algorithm capable of
automating the verification of examples given in the prior work, and
our extensions in this work additionally enable the type-based
verification of noninterference modulo output for more complex secret
sharing protocols,e.g., that leverage Beaver triples (see
...
Bit-Vector theory for binary circuits.
...
The current work extends previous work \cite{skalka-near-esop25} as
discussed above. Key differences as noted in Figure
\ref{fig-comp-wrap} include a Hoare logic and a more expressive
confidentiality type system supporting analysis of more complex
programs that enforces passive security in the real/ideal model, vs.~a
weaker gradual release property.
Furthermore our Hoare logic analysis has been fully implemented in the
current work in an Antlr/Scala/cvc5-based framework.

Besides \cite{skalka-near-esop25} the most closely related work is
\cite{skalka-near-ppdp24}, where foundations for $\minicat/\metaprot$
are introduced including a verification algorithm. However, this
algorithm only works for $\mathbb{F}_2$-- e.g., binary circuits-- and
does not scale due to exponential complexity, necessitating
semi-automated proof techniques. In contrast, our type systems scale
to arbitrary prime fields and whole program analysis for larger
circuits (protocols). So our work is a significant advancement of
$\minicat/\metaprot$.  Our work is also closely related to
Satisfiability Modulo Finite Fields \cite{SMFF} in that we use it as
an enabling technology as discussed in Section \ref{section-smt} and
subsequently.
...
type system has been defined as an embedded DSL \cite{6266151}
that enforces a version of noninterference that is sound for passive
