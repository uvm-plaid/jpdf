...
input $\phi$ and randomness $\rx{x}{\cid}$ and produces
...
\begin{definition}
  ...
  $.
  Given pmf $\pmf$ and $\Phi$ with $\rvs(\pmf) = \bigcup_{\phi \in \Phi} \dom(\phi)$,
  for all $\store \in \mems(\Phi)$ define:
%$$
%\pmf(\phi = v) \defeq \sum_{\store \in \mems(X) \ \text{with}\ \store(\phi) = v} \pmf(\store)
  %$$
  \cnote{THERE IS A PROBLEM WITH THIS! Should be marginalized on $\Phi$ not $X$...}
  $$
...
\begin{definition}
  ...
  of $X$ given $\Phi$ where $\bigcup_{\phi \in \Phi} \dom(\phi) \cup X = \rvs(\pmf)$ 
...
shares which generate shares in uniform marginal distributions. Here
and later we leverage (conditional) probabilistic independence as
...
\begin{definition}
  ...
  \begin{eqnarray*}
    ...
    \Phi_b & \defeq & \setit{ \phi_b \mid ([\phi_1,\phi_2] : \sharety) \in \Gamma} \cup
                        \setit{ \phi_a \mid (\pubx{\recon(\phi_1,\phi_2)} : \sty) \in \Gamma} \\% \Phi_a^m \cup \Phi_a^p \\
  ...
  \item For all $a,b \in \setit{1,2}$ we have
    $
    \condsep{\progtt(\prog)}{(S_{\setit{b}} \cup \Phi^o)}{\Phi_b}{S_{\setit{a}}}
    $.
    %$
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup \Phi^o}
    %= 
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup  \Phi_a \cup \Phi^o}
    %$.
    \item For all $a,b \in \setit{1,2}$ and $([\phi_1,\phi_2] : \sharety) \in \Gamma$ we have:
    $$\condsep{\progd(\prog)}{(S_{\setit{b}} \cup  \Phi_b \cup \Phi^o \cup \setit{\recon(\phi_1,\phi_2)})}{\phi_a}{S_{\setit{a}}}$$
    %$$\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup  \Phi_a \cup \Phi^o \cup \setit{\recon(\phi_i^1,\phi_i^2)}} =
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}}  \cup  \Phi_a \cup \Phi^o \cup \setit{\recon(\phi_i^1,\phi_i^2), \phi_i^b}}$$
...
\begin{lemma}
  ...
  Then for all $a,b \in \setit{1,2}$ with $a \ne b$ the following conditions hold:
    ...
    \item
      $
      \condsep{\progtt(\prog)}{(S_{\setit{b}} \cup \Phi^o)}{(\Phi_b \cup \setit{\phi_b})}{S_{\setit{a}}}
      $
    %$
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup \Phi^o}
    %= 
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup  \Phi_a \cup \setit{\phi_a} \cup \Phi^o}
    %$
    \item
       $\condsep{\progtt(\prog)}{(S_{\setit{b}} \cup  \Phi_b \cup \Phi^o \cup \setit{\recon(\phi_1,\phi_2)})}{\phi_a}{S_{\setit{a}}}$
    %$\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}} \cup  \Phi_a \cup \Phi^o \cup \setit{\recon(\phi_1,\phi_2)}} =
    %\condd{\progtt(\prog)}{S_{\setit{a}}}{S_{\setit{b}}  \cup  \Phi_a \cup \Phi^o \cup \setit{\recon(\phi_1,\phi_2), \phi_b}}$
    \end{enumerate}
\end{lemma}
\begin{proof}
  The proof is by induction on the derivation of $\Gamma, \toeq{\prog} \vdash [\phi_1,\phi_2] : \sharety$.
...
\begin{proof}
  ...
  with $H = \setit{a}$ and $C = \setit{b}$ for $a,b \in \setit{1,2}$.
  ...
  %\ref{lemma-OK}.
  Let $\Phi_b$ and $\Phi^o$ be as defined in Definition \ref{def-OK}. 
  By Definition \ref{def-sty-closed} we have that $x \in \houtputs$
  implies $x \in \Phi_b$ and $(\out{\cid} : \outty) \in \Gamma$ for all
  $\out{\cid} \in O$. 
  Therefore by Definition \ref{def-OK} (and marginalizing away
  extraneous $(\stt : \sty) \in \Gamma$) we have:
  $$
  \condsep{\progd(\prog)}{(S_C \cup \Phi^o \cup O)}{\houtputs}{S_H}
  $$
  By definition of $\TirName{Output}$ it is also the case that for all
  $\out{\cid} \in O$ we have $\toeq{\prog} \models o \eop \phi$ for some
  $\phi \in \Phi^o$. Therefore since we can assume wlog that all revealed
  values are assigned to outputs, we have
  $
  \condsep{\progd(\prog)}{(S_C \cup O)}{\houtputs}{S_H}
  $,
  so by definition we have that for all $\store \in \runs(\prog)$: 
  $$
  \condd{\progd(\prog)}{S_H}{S_C \cup O}(\store) = \condd{\progd(\prog)}{S_H}{S_C \cup \houtputs \cup O}(\store)
  $$
