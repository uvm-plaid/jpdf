\newcommand{\compwrapfig}
{
  \begin{figure}[t]
    \begin{tabular}{lccccccc}
      & 
      \begin{sideways} probabilistic features \end{sideways} &
      ...
      Swanky \cite{8429300} &  \checkmark & &  \checkmark & & & & \checkmark \\\hline
      $\metaprot$/$\minifed$ 1 \cite{skalka-near-ppdp24} & \checkmark & \checkmark & * & * & * & \checkmark & * \\\hline
      $\metaprot$/$\minifed$ 2 \cite{skalka-near-esop25} & \checkmark & \checkmark & \checkmark & * & * & \checkmark & * \\\hline
...
$$
    \begin{array}{rcl@{\hspace{4mm}}r}
      \multicolumn{4}{l}{v \in \fieldp{p},\ w \in \mathrm{String},\ \cid \in \mathrm{Clients} }\\[2mm] %, \bop \in \{ \eand, \eor, \exor \}} \\[2mm]
      ...
      x &::=& \elab{\flip{w}}{\cid} \mid \elab{\secret{w}}{\cid} \mid \elab{\mesg{w}}{\cid} \mid  \rvl{w} \mid \out{\cid} & \textit{located variables} \\[1mm]
...
\begin{fpfig}[t]{Semantics of $\minicat$ expressions (top) and programs (bottom).}{fig-minicat-redx}
 $$
...
\newcommand{\toeqfig}{
\begin{fpfig}[t]{Interpretation of $\minicat$ expressions (top) and programs (bottom) as
...
\begin{mathpar}
  ...
  \inferrule
      ...
      {\cheatj{\Delta; x : \ity{\cid}{V}}{H,C}{\extend{\seclev}{x}{\hilab \wedge (\bigwedge_{x \in V} \seclev(x))}}}
...
\begin{array}{rcl@{\hspace{7mm}}r}
  ...
  e &::=& \mv \mid \flip{e} \mid \secret{e} \mid \mesg{e} \mid \rvl{e} \mid \outkw \mid e \bop e 
   \mid y \mid \bv{e,\ldots,e} \mid & \gdesc{expressions}\\
  ...
  \bop &::=& \fplus \mid \fminus \mid \ftimes \mid \oplus \mid \ominus \mid \concat  \\[1mm]% \textit{operators}\\[2mm]
...
$$
  \begin{array}{rclr}
    \peq &::=& e \eop e \mid \notg{\eqs} \wedge \notg{\eqs} \mid \notg{\eqs} \vee \notg{\eqs}
    \mid \notg{\eqs} \impl \notg{\eqs} \mid \neg\notg{\eqs} & \qquad \textit{abstract constraints}%\\
    %C &::=& [\,] \mid \bv{\phi_1,\ldots,\phi_j,C,e_1,\ldots,e_k} \mid C \bop C & \textit{constraint contexts}
  \end{array}
$$
\begin{mathpar}
  \inferrule
      {e_1 \redx \phi_1 \cdots e_n \redx \phi_n}
      {\bv{e_1,\ldots,e_n} \redx \bv{\phi_1,\ldots,\phi_n}}
...
\begin{fpfig}[t]{Average verification times for $\metaprot$ binary and arithmetic circuit libraries and small circuits over 20 trials, in seconds. Cells marked with * were limited to a single trial due to excessive verification times. Experiments that timed out did not finish within 8 hours.}{fig-preludetest}
\begin{center}
\begin{tabular}{c}
\begin{tabular}{l|c|c|c|c|c}
         & Bit-Vector & SMFF $\fieldp{2}$ & SMFF $\fieldp{2^{17} - 1}$ &  SMFF $\fieldp{2^{31} - 1}$ &
      SMFF $\fieldp{2^{61} - 1}$ \\
  \hline
  \text{Passive Beaver Library} & 1.012 & .911 & 1.135 & 1.114 & 1.059 \\
  \text{Malicious Beaver (BDOZ) Library} & 1.175 & 1.158 & 1.182 & 1.153 & 1.178 \\
  \text{GMW Library} & .932 & .936 \\ % & N/A & N/A & N/A \\
\end{tabular}\\[10mm]
\hline\\
\begin{tabular}{l|c|c|c}
  &  \begin{tabular}{l}Bit-Vector,\\Compositional\\(this work)\end{tabular} &
       \begin{tabular}{l} Bit-Vector,\\Whole-Program\\(prior work~\cite{skalka-near-ppdp24})\end{tabular} & \begin{tabular}{l} SMFF $\fieldp{2}$\\(this work)\end{tabular} \\
  \hline
  \text{GMW 2-Bit Zero Test (3 Gates)} & .037 & .029 & 261.098 \\
  \text{GMW 2-Bit Addition (4 Gates)} & .038 & .032 & 261.098 \\
  \text{GMW 8-Bit Zero Test (15 Gates)} & .051 & .057 & Timeout \\
  \text{GMW 64-Bit Zero Test (127 Gates)} & 0.308 & 1.071 & Timeout \\
  \text{GMW 64-Bit Addition (376 Gates)} & 5.735 & 4623.666* & Timeout \\
  \text{GMW 64-Bit Subtraction (439 Gates)} & 9.985 & 10661.325* & Timeout \\
\end{tabular}
\end{tabular}
%\begin{tabular}{r|c|c|c|c|c}
%  \textit{SMT theory} & Bit-Vector & SMFF $\fieldp{2}$ & SMFF $\fieldp{2^{17} - 1}$ &  SMFF $\fieldp{2^{31} - 1}$ &
%      SMFF $\fieldp{2^{61} - 1}$ \\
%  \hline
%  \text{Passive Beaver Library} & 1.012 & .911 & 1.135 & 1.114 & 1.059 \\
%  \text{Malicious Beaver (BDOZ) Library} & 1.175 & 1.158 & 1.182 & 1.153 & 1.178 \\
%  \text{GMW Library} & .932 & .936 \\ % & N/A & N/A & N/A \\
%  \text{GMW 2-Bit Zero Test (3 Gates)} & 1.134 & 261.098 \\ % & N/A & N/A & N/A \\
%  %\text{GMW 4-Bit Zero Test (7 Gates)} & 1.154 & Unknown \\ % & N/A & N/A & N/A \\
%  \text{GMW 8-Bit Zero Test (15 Gates)} & 1.250 & Unknown \\ % & N/A & N/A & N/A \\
%  \text{GMW 64-Bit Zero Test (127 Gates)} & 6.550 & Unknown \\ %& N/A & N/A & N/A 
%\end{tabular}
\end{center}
\end{fpfig}
}

\newcommand{\solvetimescatterfig}{
\begin{fpfig}[t]{Scatter plot of verification solve times.}{fig-solve-time-scatter}
\begin{center}
\includegraphics[width=\linewidth]{external/solve_time_scatter.pdf}
\end{center}
\end{fpfig}
}
