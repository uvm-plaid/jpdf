...
In Section \ref{section-smt} we gave the example of 3-party additive sharing
for any field $\fieldp{p}$, in Section \ref{section-sty} we discussed the
example of YGC input encoding, and in Sections \ref{section-metalang} and
\ref{section-metaty} we presented and discussed the example of gates in the GMW
protocol for binary circuits. In this section we present an extended example of
2-party arithmetic circuits with multiplication gates defined using Beaver
triples \cite{evans2018pragmatic}. We first define a compositional gate library
in $\metaprot$ with correctness conditions defined using the Hoare logic
developed in Section \ref{section-metalang}, along with a small example circuit
(Figure \ref{fig-beaver}). Then we extend the library with type annotations
illustrating the use of share types developed in Sections \ref{section-sty} and
\ref{section-metaty} to illustrate how confidentiality properties can be
enforced (Figure \ref{fig-beaver-types}); this annotated variant is
illustrative and is not part of the current implementation. Lastly, we extend
the base library with additional pre and postcondition annotations to enforce
...
bit, 32 bit, and 64 bit integers. We ran all experiments on an Intel(R)
Core(TM) Ultra 7 255H CPU. Results are presented in Figure \ref{fig-preludetest}.

\preludetestfig

We found that verification of library components was efficient even up to large
field sizes. Since typing is compositional (see, e.g., Figure
\ref{fig-beaver-types} for an illustrative extension), verification of gate
properties relevant to \emph{security} should scale to larger circuits.

For an empirical comparison of how compositionality can reduce solve time in
GMW arithmetic circuits, we ran the experiments shown in Figure
\ref{fig-preludetest} with and without customized postconditions. Omitting
these postconditions reduces verification to the prior whole-program analysis
approach of Overture~\cite{skalka-near-ppdp24}, since the solver can no longer reuse component summaries
and must verify constraints for the fully expanded circuit directly.
%
This change enables a direct comparison against our prior approach.
%
The
scatter plot shown in Figure \ref{fig-solve-time-scatter} demonstrates that the
compositional use of customized postconditions improves verification time by
orders of magnitude, especially for 64-bit addition and subtraction, which have
the most gates of all the circuits we tested. The SMFF solver, however, fared
poorly in all these circuits. We believe the optimizations that make the
bit-vector solver efficient in these cases are unavailable to the SMFF solver,
even when it runs in $\mathbb{F}_2$. In all but the simplest cases, it did
not finish.
...
performance than previous brute-force whole program methods developed for binary
...
implements a federation-wide reveal of a shared value through
...
\subsection{Share Type Annotations (Illustrative).}

Figure \ref{fig-beaver-types} presents an illustrative extension of our
previous gate definitions with share type annotations to enforce
confidentiality. This annotated variant is not part of the current
implementation. The core idea is that wire values must be \ttt{share}s. The
\ttt{genshares} function includes an annotation specifying that all input gates
establish the $\ttt{shares}$ invariant for wire values. In the \ttt{sum}
function, the \ttt{share} type preconditions for input wires are specified. The
output wire shares can be declared as \ttt{share}s without any coercion needed,
since given preconditions the typing judgment ($\Vdash$) can check the use of
