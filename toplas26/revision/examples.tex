...
\emph{security} will scale to larger circuits. For an emperical comparison, we
ran the experiments shown in Figure \ref{fig-preludetest} with and without
customized postconditions. The scatter plot shown in Figure
\ref{fig-solve-time-scatter} demonstrates how customized postconditions can
improve verification time by orders of magnitude, especially for 64-bit
addition and subtraction, which have the most gates of all the circuits we
tested.
