...
\emph{security} will scale to larger circuits. For an empirical comparison, we
ran the experiments shown in Figure \ref{fig-preludetest} with and without
customized postconditions. Omitting these postconditions reduces verification
to the prior whole-program analysis approach of Overture, since the solver can
no longer reuse component summaries and must verify constraints for the
fully expanded circuit directly. The scatter plot shown in Figure
\ref{fig-solve-time-scatter} demonstrates that the compositional use of
customized postconditions improves verification time by orders of magnitude,
especially for 64-bit addition and subtraction, which have the most gates of
all the circuits we tested.
