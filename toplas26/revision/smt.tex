...
programming. In this paper we explore using SMT instead which
has several advantages. SMT is a highly studied and optimized
system with a variety of theories. In particular, we can leverage
the cvc5 Bit-Vector theory for verification in $\fieldp{2}$
and extend verification to abritrary prime fields using 
Satisfiability Modulo Finite Fields \cite{SMFF}. As we will
show in subsequent sections, this interpretation supports automated
enforcement of correctness guarantees as well as static type analysis
for enforcing confidentiality and integrity properties.  In this
section we formulate a Hoare Logic for $\minicat$ as a foundation for
protocol verification.

\subsection{Constraint Satisfiability and Entailment}

We introduce the following syntax of SMT-style constraints for
$\minicat$. The language of constraint terms includes field values
and operations ($+$, $-$, and $*$), and also bit operations
(construction $\bv{\cdot}$, summation $\oplus$, and multiplication
$\otimes$). We note that bit vector operations are included \emph{only}
in the constraint language-- they allow us to leverage cvc5 bit vector
operations and constraints, while protocols (circuits) themselves
still manipulate individual bits. We illustrate how this can be used for
verification in Section \ref{section-examples}.
$$
\begin{array}{rclr}
  \phi &::=& v \mid x \mid \phi \fplus \phi \mid \phi \fminus \phi \mid \phi \ftimes \phi
     \mid \bv{\phi_1,\ldots,\phi_n} \mid \phi \oplus \phi \mid \phi \otimes \phi & \textit{constraint terms}\\ 
...
%$\eqs$, where
While constraint terms $\phi$ are similar to expressions $\be$,
in contrast they can ``cross party lines''. This is needed
...
SMT, in particular the cvc5 Bit-Vector theory in the case of $\fieldp{2}$ and
Satisfiability Modulo Finite Fields \cite{SMFF} for other prime fields. We can
leverage this to implement a critical \emph{entailment} property,
written $\eqs_1 \models \eqs_2$.  Our entailment relation is based on
satisfiability in a standard sense, where we represent models as
memories $\store$ (mappings from variables to values).
\begin{definition}
...
publicly reveals the sums of all their shares, and the sum of those is
...
To reason effectively about pre and postconditions we introduce
