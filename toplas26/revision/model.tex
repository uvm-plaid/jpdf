...
\begin{definition}
  A \emph{probability mass function} $\pmf$ over random variables $X$
  (written $\rvs(\pmf) \defeq X$) is a function
  with domain $\mems(X)$ and range $\mathbb{R}$ such that:
  $$
  ...
  %We write $\rvs(\pmf) = X$ to denote the variables
  %over which $\pmf$ is defined. 
\end{definition}
...
\begin{definition}
  Given $\pmf$ with $\rvs(\pmf) = X_2$, the \emph{marginal distribution}
...
\begin{definition}
  ...
  of $X_1$ given $X_2$ where $X_1 \cup X_2 = \rvs(\pmf)$ 
  ...
  \begin{cases}
    \begin{array}{ll}
      ...
      \pmf(\store) / \margd{\pmf}{X_2}(\store_{X_2})\ & \text{\ o.w.}
     \end{array}
  \end{cases}
  $$
\end{definition}
The first case applies when the marginal probability that $X_2$ takes
the value assigned by the restricted memory $\store_{X_2}$ is zero;
without this case, the conditional probability would be undefined.
We also define some convenient syntactic sugaring for application of
pmfs to realizations when their domains don't match.
%The first will allow us to
...
\begin{definition}
  Given $\pmf$ and $\store$ with $\rvs(\pmf) \subseteq \dom(\store)$,
  $\pmf(\store) \defeq \pmf(\store_{\rvs(\pmf)})$.
  %\defeq \margd{\pmf}{\dom(\store)}(\store)$
  %if $\dom(\store) \subset \rvs(\pmf)$, and define $\pmf(\store) \defeq \pmf(\store_{\rvs(\pmf)})$ if
  %$\dom(\pmf) \subset \dom(\store)$.
  %and
  %$\store_2 \in \mems(X_2)$ define
  %$\pmf(\store_1)  \defeq \margd{\pmf}{X_1}(\store_1)$ 
  %\begin{enumerate}
...
%  \pmf(\{ x \mapsto 1\}\ |\ \{ y \mapsto 1 \}) = 1$.
  We write $\condsep{\pmf}{X_1}{X_2}{X_3}$ iff for all $\store \in \mems(X_1 \cup X_2 \cup X_2)$
  we have $\condd{\pmf}{X_2 \cup X_3}{X_1}(\store)
  = \condd{\pmf}{X_2}{X_1}(\store) * \condd{\pmf}{X_3}{X_1}(\store)$. If $X_1$ is empty
...
\begin{definition}
  ...
  \begin{array}{c}
    ...
    \store \in \aruns(\prog) \wedge \{ x_1,\ldots,x_n\} = \iov(\prog) - \dom(\store) \}
  \end{array}
  $$
\end{definition}
...
\begin{definition}[Corrupt and Honest Views]
  ...
  define $\views(\prog) \defeq M \cup P$. Letting $I_1$, $I_2$ be
  a partitioning of the federation clients, define:
  $$
  \begin{array}{lcl}
    V_{I_1 \rhd I_2} & \defeq
        & \{\ \rvl{w} \mid\ \reveal{w}{\be}{\cid} \in \prog \wedge \cid \in I_1 \ \}\ \cup\ %\\
    % & &
    \{\ \elab{\mesg{w}}{\cid}\ \mid\  \eassign{\mesg{w}}{\cid}{\be}{\cid'} \in
           \prog \wedge \cid \in I_2 \wedge \cid' \in I_1 \ \} 
  \end{array}
  $$
  Given a partitioning of the federation $H,C$, we call $\houtputs$
  \emph{corrupt views} and $\cinputs$ \emph{honest views}.
\end{definition}
...
their outputs-- that equal the ideal functionality
$\idealf$ applied to all secret inputs-- they do not enjoy strict
noninterference. Public reveals and protocol outputs are fundamentally
forms of declassification. But the following property of probabilistic
noninterference \emph{conditioned on output} is sound for passive
security. Intuitively, in this definition we can think of
$\condd{\progtt(\prog)}{S_H}{S_C \cup O}$ as the distribution of
honest inputs $S_H$ given the adversarial knowledge of corrupt inputs
$S_C$ and public outputs $O$. This establishes an upper bound on
information that can be leaked to the adversary as determined by the
ideal functionality under the assumption of corrupted parties-- or,
put another way, the adversary's ability to guess $S_H$ knowing $O$
and $S_C$.  We can think of $\condd{\progtt(\prog)}{S_H}{S_C \cup O
  \cup \houtputs}$ as the distribution of $S_H$ given the same corrupt
inputs and public outputs, and additionally the adversarial views
$\houtputs$ communicated during runs of $\prog$. If these
distributions are the same, we can deduce that the protocol reveals no
more information about $S_H$ than the corrupt inputs $S_C$ and public
outputs $O$ alone. More details and a proof that this property is
sound for passive simulator security are reported in previous work
\cite{skalka-near-ppdp24}.
\begin{definition}[Noninterference modulo output]
  ...
  iff for all $H$ and $C$ and $\store \in \runs(\prog)$ 
  we have:
  $$
  \condd{\progtt(\prog)}{S_H}{S_C \cup O}(\store) = \condd{\progtt(\prog)}{S_H}{S_C \cup O \cup \houtputs}(\store)
 $$
\end{definition}

In practice, MPC protocols typically satisfy a \emph{gradual release}
property \cite{sabelfeld2009declassification}, where the distribution
of messages exchanged during the protocol are independent of the
distribution of input secrets, with only final reveal and output
stages having dependence on input secrets. Referring to Example
\ref{section-smt-example}, the messages exchanged during the protocol
in this case are all in independent uniform distributions, while the
broadcast reveals $\rvl{1-3}$ used to reconstruct the outputs
$\out{1-3}$ are dependent. Gradual release is strictly weaker than
noninterference. It says that intentional reveals and outputs
(declassifications) are the only point of information escape in the
protocol, but does not specify what information may be revealed.
\begin{definition}
...
Noninterference modulo output directly implies passive simulator
security~\cite{skalka-near-ppdp24}, so a sound analysis for
noninterference modulo output is also a sound analysis for passive
simulator security. However, such an analysis is not necessarily
complete: there exist MPC protocols that satisfy passive simulator
security, but which do not satisfy noninterference modulo output. In
particular, our definition of noninterference modulo output requires
equality of the two distributions, which does not hold in protocols
for which indistinguishability relies on computational hardness
assumptions (this includes protocols based on public-key cryptography,
such as many protocols for oblivious transfer). Our approach is not
capable of proving security for these protocols.
...
\begin{definition}[Integrity]
  ...
  if $\store \in \aruns(\prog)$ there exists $\store' \in \runs(\prog)$ with
  $\store_{S_H \cup X}' = \store_{S_H \cup X}$ and:
    $$
    \condd{\progtt(\prog,\adversary)}{X}{S_H \cup Y}(\store) =
    \condd{\progtt(\prog)}{X}{S_H \cup Y}(\store')
    $$ 
  where $X \defeq (\houtputs \cup O_H) \cap \dom(\store)$ and $Y = \cinputs \cap \dom(\store)$. 
\end{definition}
