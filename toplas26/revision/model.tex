...
$\condd{\progtt(\prog)}{S_H}{S_C \cup O}$ as the distribution
of honest inputs $S_H$ given the adversarial knowledge of
corrupt inputs $S_C$ and public outputs $O$. This establishes
an upper bound on information that is allowed to be leaked to the
adversary as determined by the ideal functionality under the
assumption of corrupted parties-- or, more precisely, the
adversary's ability to guess $S_H$. We can think of
$\condd{\progtt(\prog)}{S_H}{S_C \cup O \cup \houtputs}$
as the distribution of $S_H$ given the same corrupt inputs
and public outputs, and additionally the adversarial views
$\houtputs$ communicated during runs of $\prog$. If these distributions
are the same, we can deduce that the protocol reveals no
more information about $S_H$ than the corrupt inputs $S_C$
and public outputs $O$ alone. More details and a proof
that this property is sound for passive simulator security
are reported in previous work \cite{skalka-near-ppdp24}.
