...
the instantiation of $\notg{R}$ with linearity enforced. A practical
programming detail is where the share type environment precondition
$\notg{\Gamma_1}$ is defined.  In our implementation, we write these
as an additional component of \ttt{precondition} annotations, as
illustrated in extended examples discussed in Section
...
\begin{theorem}
  ...
  and $\toeq{\progpre} \models \eqspre$. Then $(\progpre;\prog),\Gamma_2,R : {OK}$.
\end{theorem}
...
protocol to be defined in detail, just (weakened) postconditions of it
that are sufficient to verify the protocol being programmed.

Theorem \ref{theorem-metaty} also does not provide a complete recipe
for obtaining noninterference modulo output-- as suggested by Theorem
