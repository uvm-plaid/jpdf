...
interaction with the Satisfiability Modulo Fields theory.  To evaluate
performance, we have written and verified gate libraries and small
circuits using the GMW protocol for binary circuits as excerpted in
...
integers. Results are presented in Figure \ref{fig-preludetest}. In
summary, verification is efficient, largely due to the performance of
underlying cvc5 verification, with categorically better performance
than can be achieved using previous brute-force methods developed in
...
summing a wire value $\ttt{RECON(x)}$ and a random Beaver triple value
$\ttt{r[x++a]@3}$, the federation can publicly open the value since it is
...
respectively, and the corresponding key is $\ttt{m[x++"k"]@2}$ is held
by client \ttt{2}. The correctness condition of this 3-tuple is
$\ttt{MACOK}(x,2,1)$. The same is true symmetrically, so $\ttt{MACed}$
establishes both sides of the correctness condition.  In general,
these are used in gate preconditions to assume that the MAC conditions
hold for input shares, and output shares are $\ttt{MACed}$. The
homomorphic properties that preserve $\ttt{MACed}$ness, especially in
\ttt{mult}, are perhaps even more complex than the algebraic
correctness of gates, so it is very beneficial to verify them
compositional especially for large circuits.
