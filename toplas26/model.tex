...
\begin{definition}
  ...
  of $X_1$ given $X_2$ where $X_1 \cup X_2 = \dom(\pmf)$ 
...
\begin{definition}
  Given $\pmf$ and $\store$ with $\dom(\pmf) \ne \dom(\store)$,
  $\pmf(\store) \defeq \margd{\pmf}{\dom(\store) \cap \dom(\pmf)}(\store_{\dom(\store) \cap \dom(\pmf)})$.
  %\defeq \margd{\pmf}{\dom(\store)}(\store)$
  %if $\dom(\store) \subset \dom(\pmf)$, and define $\pmf(\store) \defeq \pmf(\store_{\dom(\pmf)})$ if
