...
\begin{definition}
  ...
  $$
\end{definition}
We also define some convenient syntactic sugaring for ``implicit
marginalization'' given a realization.
...
\begin{definition}
  Given $\pmf$ and $X \subseteq \dom(\pmf)$, for all $\store \in \mems(X)$
  define $\pmf(\store)  \defeq \margd{\pmf}{X}(\store)$.
  %and
...
\begin{definition}[Noninterference modulo output]
  ...
  we have:
  $$
  \condd{\progtt(\prog)}{S_H}{S_C \cup O} = \condd{\progtt(\prog)}{S_H}{S_C \cup O \cup \houtputs}
 $$
\end{definition}
...
\begin{definition}[Integrity]
  ...
  if $\store \in \aruns(\prog)$ we have:
    $$
