...
We define the federation as $\setit{1,2}$ (client 1 and client 2).
...
guarantees confidentiality (Theorem \ref{theorem-OK}).
...
\begin{proof}
  ...
  By Definitions \ref{def-sty-closed} we have that $x \in \houtputs$
  ...
  $$
  \condd{\progd(\prog)}{S_H}{S_C \cup O} = \condd{\progd(\prog)}{S_H}{S_C \cup V_C \cup \Phi^p \cup O}
  $$
  By definition of $\TirName{Output}$ it is also the case that for all
  $\out{\cid} \in O$ we have $\toeq{\prog} \models o = \phi$ for some
