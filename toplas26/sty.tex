...
Our approach to static confidentiality analysis is to enforce the use of sound
techniques in semi-homomorphic encryption in protocols.  As discussed in
...
shares). To comport with this we introduce distinct judgement forms for
...
clarity of presentation and the analysis can be generalized to more parties.
...
$(\genshares,\recon)$ which generate and reconstruct shares,
...
$$
In order for $\genshares$ to be \emph{sound}, we require
...
by $\genshares$, and adds $\rx{y}{\cid_1}$ to the set of variables
consumed by $\genshares$, ensuring one-time usage. The
\TirName{Reveal} rule generates the binding $\pubx{\recon(\px{x},\px{y})} :
\outty$ with the requirement that $[\px{x},\px{y}]$ are typable as
shares. The \TirName{SecOpen} rule is similar except it allows the
assertion $\pubx{\recon(\px{x},\px{y})} : \pubty$ if we can additionally
...
So we can define:
\begin{eqnarray*}
  \genshares(\phi,\rx{y}{\cid}) &\defeq& \phi - \rx{y}{\cid}, \rx{y}{\cid}\\
...
This approach highlights an imporant interaction between our type
...
To proceed, we define a well-formedness property of tuples $\prog,\Gamma,R$
...
Property (ii) ensures the key property of sharing-- that individual shares
...
\begin{definition}
  ...
  \end{enumerate} 
\end{definition}
With the definition of ${OK}$ in place, we can define what we mean by soundness
of $\genshares$. In this definition, the first condition establishes that
initial shares are independent of honest secrets, and the second establishes
that they reveal no more than their reconstruction. These conditions are
obviously met by ``vanilla'' additive or Shamir shares which generate
shares in uniform marginal distributions.
\begin{definition}
  \label{def-genshares}
...
\begin{lemma}
  ...
  Given $\prog,\Gamma,R : {OK}$ where $\secrets(\prog) = S$ and:
  $$
  \Gamma = \setit{[\phi_1^1,\phi_1^2] : \sharety, \ldots, [\phi_n^1,\phi_n^2] : \sharety}
  \cup \setit{\pubx{\phi_1} : \pubty,\ldots,\pubx{\phi_j} : \pubty} \cup
  \setit{\pubx{\phi^o_1} : \outty,\ldots,\pubx{\phi^o_k} : \outty}
  ...
  Then for all $a,b \in \setit{1,2}$ the following conditions hold:
    ...
    \condd{\progtt(\prog)}{S_{\setit{\cid_a}}}{S_{\setit{\cid_b}} \cup \setit{\phi_1^b,\ldots,\phi_n^b,\phi^b} \cup
      % \setit{\phi_1,\ldots,\phi_j}
      \setit{\phi^o_1,\ldots,\phi^o_k}}$
    \item $\condd{\progtt(\prog)}{S_{\setit{\cid_a}}}{\setit{\recon(\phi_i^1,\phi_i^2)}} =
      \condd{\progtt(\prog)}{S_{\setit{\cid_a}}}{\setit{\recon(\phi_i^1, \phi_i^2), \phi_i^b}}$
    \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  semi-homorphic properties of secret sharing \cite{10.1007/978-3-030-68869-1_3}.
...
\begin{proof}
  ...
  \ref{lemma-sty-noninteractive-sound} relevant conditions (i-ii), and the
  the requirement that the public value be masked with an OTP satisfies conditions
...
shares. We capture this formally with the following notion of
...
\begin{proof}
  ...
  %\ref{lemma-OK}.
  By Definitions \ref{def-sty-closed} we know that every $x \in \houtputs$
  occurs in some $\stt$ with $(\stt, \sharety) \in \Gamma$, and
  that $(\out{\cid} : \outty) \in \Gamma$ for all $\out{\cid} \in O$.
  Therefore by Definition \ref{def-OK} (and marginalizing away
  extraneous $(\stt : \sty) \in \Gamma$) we have:
  $$
  \condd{\progd(\prog)}{S_H}{S_C \cup O} = \condd{\progd(\prog)}{S_H}{S_C \cup V_C \cup O}
  $$
