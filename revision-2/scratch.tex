...
\cnote{Need to fix the following to allow reduction of $x$.}
\begin{mathpar}
  \inferrule[Mesg]
...
\begin{mathpar}
  \inferrule[Sig]
            {\codebase(f) = x_1,\ldots,x_n, \instr \\
             \subn = [\mv_1/x_1]\cdots[\mv_n/x_n] \\
             \mtj{\varnothing}{\subn(\eqs_1)}{\subn(\instr)}{\subn(\Gamma)}{\subn(R)}{\eqs} \\ \eqs \models \subn(\eqs_2)}
            {f : \fty{\eqs_1}{x_1,\ldots,x_n}{\Gamma}{R}{\eqs_2}}
\end{mathpar}
...
\begin{verbatimtab}
    ...
    andgmw(z, x, y) {
      ...
      m[z]@2 := OT4(m[x],m[y],table,2,1)
        as ~((m[x]@1 xor m[x]@2) and (m[y]@1 xor m[y]@2)) xor r[z]@1);
      m[z]@1 := r[z]@1
    }
