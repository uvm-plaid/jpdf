...
\begin{definition}
  Given $H,C$ and $\prog$ with $\secrets(\prog) = S$ and $x \in \views(\prog)$,
  we say $x$ has an \emph{aberration} iff
  $\exists \adversary, \store \in \botruns(\prog)$ such that $\store(x) \ne \bot$
  and $\neg\exists \store' \in \runs(\prog) .
  \store_{S_H \cup \setit{x}} = \store'_{S_H \cup \setit{x}}$.
\end{definition}

\begin{lemma}
  If $\prog$ with $\views(\prog) = V$ does not have integrity for some
  $H,C$, then there exists $x \in \houtputs$ with an
  aberration.
\end{lemma}

\begin{lemma}
  Given $\prog$ and $H,C$, if $x$ has an aberration then one of
  ...
  \item There exists $\xassign{x}{\be}{\cid} \in \prog$ with $\cid \in H$ and
    $x' \in \vars(\be)$ such that $x'$ has an aberration.
  \end{enumerate}
\end{lemma}
...
\begin{lemma}
  ...
  and $x$ has an aberration for some $H,C$,
