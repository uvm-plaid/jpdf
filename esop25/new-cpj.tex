\newcommand{\Shares}{\mathcal{S}}
\newcommand{\sharet}[2]{\mathit{share}(#1,#2)}
\newcommand{\sid}{\texttt{s}}
\newcommand{\sexp}{s}
\newcommand{\keys}{\mathcal{K}}
\newcommand{\rkey}{\kappa}
\renewcommand{\unity}[2]{U(#1, #2)}
\newcommand{\uncorr}[2]{#1\bowtie#2}
\newcommand{\corr}{\simeq}
\renewcommand{\ty}{D}
...
\begin{mathpar}
  \inferrule[Mesg]
            {}
            {\Gamma, \varnothing, \eqs \vdash \mx{w}{\cid} : \Gamma(\mx{w}{\cid})}
            
  \inferrule[Rand]
            {}
            {\Gamma, \varnothing, \eqs \vdash \rx{w}{\cid} : \setit{\unity{\rx{w}{\cid}}{\varnothing}}}
            
  \inferrule[Ideal]
            {}
            {\Gamma, \varnothing, \eqs \vdash f(\sx{w_1}{\cid_1},\ldots,\sx{w_n}{\cid_n}) :
              \setit{f(\sx{w_1}{\cid_1},\ldots,\sx{w_n}{\cid_n})}}

  \inferrule[Enc]
            {\Gamma, R, \eqs \vdash \phi : \ty}
            {\Gamma, R;\rx{w}{\cid}, \eqs \vdash \phi \fminus \rx{w}{\cid} : \setit{\unity{\rx{w}{\cid}}{\ty}}}

  \inferrule[HEAdd]
      {\Gamma, R_1, \eqs \vdash \phi_1 : \setit{\unity{\rkey_1}{\ty_1}} \\
        \Gamma, R_2, \eqs \vdash \phi_2 : \setit{\unity{\rkey_2}{\ty_2}} \\
        \uncorr{\unity{\rkey_1}{\ty_1}}{\unity{\rkey_2}{\ty_2}}
      }
      {\Gamma, R_1;R_2, \eqs \vdash \phi_1 \fplus \phi_2 : \setit{\unity{\rkey_1 \oplus \rkey_2}{\ty_1\cup \ty_2}}}

  \inferrule[Join]
      {\Gamma, R_1, \eqs \vdash \phi_1 : \ty_1 \\ \Gamma, R_2, \eqs \vdash \phi_2 : \ty_2}
      {\Gamma, R_1;R_2, \eqs \vdash \phi_1 \bop \phi_2 : \ty_1 \cup \ty_2}
            
  \inferrule[Eq]
            {\eqs \models \phi \eop \phi' \\ \Gamma, R, \eqs \vdash \phi' : \ty}
            {\Gamma, R, \eqs \vdash \phi : \ty}          
\end{mathpar}

\begin{mathpar}
  \inferrule[Mesg]
      {\Gamma, R, \eqs \vdash \toeq{\elab{e}{\cid}} : \ty}
      {\Gamma, \eqs \vdash \xassign{x}{e}{\cid} : (\Gamma;x:\ty),R}

  \inferrule[Seq]
      {\Gamma_1,\eqs \vdash \cmd_1 : \Gamma_2,R_1 \\
        \Gamma_2,\eqs \vdash \cmd_2 : \Gamma_3,R_2}
      {\Gamma_1, \eqs \vdash \cmd_1;\cmd_2 : \Gamma_3,R_1;R_2}
\end{mathpar}
