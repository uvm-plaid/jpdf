...
\begin{proof}
  ...
  In case $M' = \varnothing$ the result follows trivially. Otherwise XXX
\end{proof}
...
\begin{theorem}
  ...
  is valid and there exists no $H,C$ such that $\leakj{\Gamma}{M_C}{T}$ with $T$ closed
  and $S_H \cap T \ne \varnothing$, then $\prog$ satisfies gradual release.
\end{theorem}
\begin{proof}
  Immediate by Lemma \ref{lemma-cpjsound} and definition of gradual release.
\end{proof}
