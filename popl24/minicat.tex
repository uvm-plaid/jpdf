...
and $V$ is the set of views in $\prog$, and we write $\flips(\prog)$
to denote the set $F$ of flip variables in $\prog$. For any set of
...
variables. As a sanity condition we will only consider safe programs
in our presentation, though in Section \ref{section-composition} we
define a type system that guarantees program safety. 
