...
$$
\config{\prog_1}{E[\ttt{andgg}(w,v_1,\ldots,v_5)]} \redxs \config{\prog_1;\prog_2}{E[v_6]}
$$

\begin{mathpar}
  \forall 1 \le i \le 6 . v_i = \{ \ttt{k} = \be_k^i; \ttt{p} = \be_p^i\}

  \forall 1 \le i \le 3 . \exists w^i_1,w^i_2 . \be_k^i \sim \flip{2}{w^i_1} \wedge  \be_p^i \sim \flip{2}{w^i_2} 

  \forall 4 \le i \le 5 . \gNIMO(\progd(\prog_1,\be_k^i), \flip{2}{w^{i-3}_1}) \wedge \gNIMO(\progd(\prog_1,\be_p^i), \flip{2}{w^{i-3}_2})
\end{mathpar}


\begin{mathpar}
  \gNIMO(\progd(\prog_1;\prog_2,\be_k^6), \flip{2}{w^3_1}) \wedge \gNIMO(\progd(\prog_1;\prog_2,\be_p^6), \flip{2}{w^3_2})
