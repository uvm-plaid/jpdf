...
noninterference. In this Section we show for to formulate extensional
properties relevant to MPC passive security in our language model by
direct appeal to program distributions. In particular,
\emph{noninterference modulo output ($\NIMO$)} is shown to be sound
for MPC passive security. In Section \ref{section-automation} we will
show how $\NIMO$ can be automatically enforced directly via
whole-program analysis, or by automatically verifying composable,
intensional properties of circuit components.
...
Example \ref{example-gmw-andcircuit} as a failure witness, where we'll
assume $C = \{0,2\}$ and $H = \{1\}$:
$$
