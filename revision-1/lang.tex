...
Eliminating cheating by adding information theoretically secure MAC
...
A field value $v$ is secret shared among $n$ clientz in BDOZ as follows.
Each client $\cid$ in a federation of size $n$ gets a value of the form
$(v_\cid,[m_1,\ldots,m_n])$, where $v_\cid$ is a secret
share of $v$, i.e., $v = \fcod{v_1 \fplus \cdots \fplus v_n}$. Further, 
values $[m_1,\ldots,m_n]$ are MACs of $v_\cid$, such that
$m_{\cid'} = k_{\cid'} + (k_\Delta * v_\cid)$ for each $\cid'\in [1..n]$,
where keys $k_{\cid'}$ and $k_\Delta$ are known only to $\cid$
and $k_\Delta$ is common to all MACs used by $\cid$. By
extending certain field operations to these forms, denoted $\macv$,
a form of semi-homorphic encryption is obtained that is adequate to compute
Beaver Triples. For example, if keys $k_1$ and $k_2$ authenticate
$\macv_1$ and $\macv_2$, then $k_1 \fplus k_2$ authenticates
$\macv_1 \macplus \macv_2$,  where $\macplus$ is addition
extended to MACed values. Relevant semantic rules for this, as
well as multiplication of MACed and field values, are given in
Figure \ref{fig-bdoz}- for more details the reader is referred to
\cite{XXX}.

We define variable forms $\macx{\secret{w}}{\cid}$ and
$\macx{\flip{w}}{\cid}$ range over local MACed shares of a secret and
random sample owned by client $\cid$, respectively. We let
$\mack{\macx{x}{\cid_1}}{\cid_2}$ denote the key that
authenticates $\cid_n$'s share of $\cid_1$'s value $x$.
We also assume a key $\flip{\ttt{delta}}$ on each client
$\cid$, which is that client's $k_\Delta$, 
In Figure \ref{fig-bdoz} we define the relevant properties
of pre-processed stores.
...
$$
Also, we can define a $\macauth$ function that authenticates
a client $\cid$'s share, and an $\macopen$ function
that extracts a field value from its shares.
\begin{mathpar}
\delta(\macauth, (v, [\ldots,m_\cid,\ldots]), k_\cid) \defeq
     (v, [\ldots,m_\cid,\ldots]) \text{\ if\ } m_\cid = \fcod{k_\cid \fplus (k_\Delta \ftimes v)}
 
\delta(\macopen, (v_1, [\ldots]), \ldots, (v_n, [\ldots]),) \defeq
\fcod{v_1 \fplus \cdots \fplus v_n}
\end{mathpar}
...
properties of Beaver Triples in Figure \ref{figure-bdoz}. First,
clients 1 and 2 compute (aka \emph{open}) $\secret{x} \fplus \flip{a}$ and
$\secret{y} \fplus \flip{b}$ where
$\secret{x}$ and $\secret{y}$ are 1 and 2's secrets respectively.
Note that both perform a \emph{secure opening} by authenticating
each others' shares and record $\secret{x} \fplus \flip{a}$ and
$\secret{y} \fplus \flip{b}$ as $\mesg{d}$ and $\mesg{e}$ respectively.
$$
\begin{array}{lcl}
  \elab{\mesg{a}}{2} &:=&
...
\begin{array}{lcl}
  ...
  \out{2} &:=& \elab{(\macopen\ \macauth(\rvl{1},\ldots) \ \macauth(\rvl{2},\ldots))}{2} 
\end{array}
$$
