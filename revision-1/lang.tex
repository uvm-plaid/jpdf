...
We let $x$ range over \emph{variables} which are
identifiers 
\begin{itemize}
  \item $S$ ranges over sets of 
\end{itemize}
We let $x$ range over variables.%, and will use $s$, $v$, and $f$ to
%range over individual secrets, views, and flips respectively.
We let $X$ range over sets of variables,
and $S,R,M,P,O$$ to range over sets of
